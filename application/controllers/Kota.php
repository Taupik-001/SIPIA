...
class Kota extends CI_Controller
{
	...
	function input()
	{
		...
		$data['nama'] = $this->session->userdata('nama');
		$data['title'] = 'Input Kota | SIPIa - Sistem Informasi Pariwisata Indonesia';
		$data["page"] = "Kota";
		$data["pageTre"] = "Master";
		$this->load->view('templates/header_admin', $data);
		$this->load->view('templates/header_main', $data);
		$this->load->view('templates/sidebar_main', $data);
		$this->load->view('kota/v_kota_input', $data);
		$this->load->view('templates/footer_copyright');
		$this->load->view('templates/footer_admin');
	}
	...
	function input_action()
	{
		...
		$config['upload_path'] = './assets/images/kota/'; //path folder
		...
		if ($_FILES['filefoto']['name']) {
			if ($this->upload->do_upload('filefoto')) {
				...
				redirect('kota'); //jika berhasil maka akan ditampilkan view vupload
			} else {
				$this->session->set_flashdata("pesan", "<div class=\"col-md-12\"><div class=\"alert alert-danger\" id=\"alert\">Gagal upload gambar !!</div></div>");
				redirect('kota/input'); //jika gagal maka akan ditampilkan form upload
			}
		}
	}
	...
	function edit($id)
	{
		...
		$data['nama'] = $this->session->userdata('nama');
		$data['title'] = 'Edit Kota | SIPIa - Sistem Informasi Pariwisata Indonesia';
		$data["page"] = "Kota";
		$data["pageTre"] = "Master";
		$this->load->view('templates/header_admin', $data);
		$this->load->view('templates/header_main', $data);
		$this->load->view('templates/sidebar_main', $data);
		$this->load->view('kota/v_kota_edit', $data);
		$this->load->view('templates/footer_copyright');
		$this->load->view('templates/footer_admin');
	}
	...
	function edit_action()
	{
		...
		$path   = './assets/images/kota/'; //path folder
		...
		if ($_FILES['filefoto']['name']) {
			if ($this->upload->do_upload('filefoto')) {
				...
				redirect('kota/edit'); //jika gagal maka akan ditampilkan form upload
			}
			...
			redirect('kota'); /* jika berhasil maka akan kembali ke home upload */
		}
	}
